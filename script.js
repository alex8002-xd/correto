...
inputBox.addEventListener('input', () => {
    ...
    // Verifica si el texto ingresado coincide con el objetivo
    if (typedText === currentSentence) {
        // Calcula el tiempo transcurrido
        const endTime = new Date().getTime();
        const timeTaken = (endTime - startTime) / 1000 / 60; // Tiempo en minutos

        // Calcula la velocidad (WPM)
        const speed = Math.round(totalWords / timeTaken);
        wpmDisplay.textContent = `WPM: ${speed}`;

        // Calcula la precisión
        const correctCharacters = typedText.split('').filter((char, index) => char === currentSentence[index]).length;
        const accuracy = Math.round((correctCharacters / currentSentence.length) * 100);
        accuracyDisplay.textContent = `Accuracy: ${accuracy}%`;

        inputBox.disabled = true; // Bloquea el campo de entrada
    } else {
        // Retroalimentación en tiempo real: Resalta errores
        const partialMatch = currentSentence.startsWith(typedText);
        inputBox.style.borderColor = partialMatch ? 'green' : 'red';
    }
});
